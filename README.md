...
Or type `make install` to install these script to `~/.vim`. 

Commands
------
|Name              | Description |
| ---------------- | ------------------- |
|CxxtagsOpenDecl   | Jump to Declaration of the item at the cursor.|
|CxxtagsOpenDef    | Jump to Definition of the item at the cursor.|
|CxxtagsListRefs   | List all the references to the message buffer.|
...
|CXXTAGS_MsgBufName   | "cxxtags_msg"     | Name of a message buffer which a reference list is printed to|
